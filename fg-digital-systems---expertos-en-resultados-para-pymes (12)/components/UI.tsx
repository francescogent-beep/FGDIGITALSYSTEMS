...
export const Button: React.FC<{
  ...
  const styles = {
    ...
    outline: "border-2 border-slate-300 text-slate-800 hover:border-blue-600 hover:text-blue-600 hover:bg-blue-50/40 focus:ring-blue-500"
  };
  ...
  return (
    <div className={`border rounded-2xl md:rounded-[1.5rem] overflow-hidden bg-white mb-4 transition-all duration-300 ${isOpen ? 'border-blue-200 shadow-lg' : 'border-slate-200 shadow-sm hover:border-slate-300'}`}>
      ...
      >
        ...
        <div className={`p-2 rounded-full transition-all duration-300 ${isOpen ? 'bg-blue-600 text-white rotate-180' : 'bg-slate-100 text-slate-500 group-hover:text-slate-700'}`}>
          <ChevronDown size={20} />
        </div>
      </button>
