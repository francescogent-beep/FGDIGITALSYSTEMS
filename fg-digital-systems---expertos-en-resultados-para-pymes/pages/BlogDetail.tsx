...
const BlogDetail: React.FC = () => {
  ...
  const seoData = METADATA[post.id] || {
    title: `${post.title} | WEB QUE VENDE`,
    description: post.excerpt,
    path: `/blog/${post.id}`,
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16">
            ...
            <div className="lg:col-span-8 px-2 md:px-0">
              <div className="rounded-[2.5rem] overflow-hidden mb-12 shadow-2xl border-8 border-slate-50">
                <img src={post.image} alt={post.title} className="w-full aspect-video object-cover" width="800" height="450" />
              </div>
              ...
              <div className="my-16 p-8 md:p-12 bg-blue-50 border border-blue-100 rounded-[2rem] flex flex-col md:flex-row items-center gap-8 shadow-sm">
                ...
                <div className="text-center md:text-left">
                  <h3 className="text-2xl font-black text-slate-900 mb-2">¿Quieres aplicar esto a tu negocio?</h3>
            ...
            <aside className="lg:col-span-3 space-y-12">
              <div className="bg-slate-900 rounded-3xl p-8 text-white shadow-2xl">
                <h4 className="text-xl font-bold mb-4">Soluciones Reales</h4>
                <p className="text-slate-400 text-sm mb-6 leading-relaxed">
                  Webs rápidas y SEO local para negocios que quieren crecer de verdad.
                </p>
                <div className="space-y-4">
                  <Link to="/servicios" className="flex items-center justify-between p-4 bg-white/5 rounded-xl hover:bg-white/10 transition-colors border border-white/10 group">
                    <span className="font-bold text-sm">Ver Servicios</span>
                    <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
                  </Link>
                  <Link to="/precios" className="flex items-center justify-between p-4 bg-white/5 rounded-xl hover:bg-white/10 transition-colors border border-white/10 group">
                  ...
                  {BLOG_POSTS.filter(p => p.id !== post.id).slice(0, 2).map(p => (
                    <Link key={p.id} to={`/blog/${p.id}`} className="group block">
                      <div className="aspect-video rounded-xl overflow-hidden mb-3">
                        <img src={p.image} alt={p.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform" width="400" height="225" />
                      </div>
