...
export const TallyModal: React.FC = () => {
  ...
  return (
    <>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-4 bg-black/40 backdrop-blur-md">
          <div 
            className="bg-white w-full h-full md:h-[85vh] md:max-w-7xl md:max-h-[90vh] md:rounded-[2rem] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.2)] flex flex-col overflow-hidden relative border-none md:border md:border-slate-200"
          >
            ...
            <div className="px-6 py-4 md:px-10 md:py-6 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
              ...
              <button 
                ...
                className="p-2 md:p-3 rounded-full bg-slate-50 text-slate-500 hover:text-slate-900 hover:bg-slate-100"
                aria-label="Cerrar"
              >
                <X size={24} />
              </button>
...
export const Button: React.FC<{
  ...
  const base = "inline-flex items-center justify-center px-6 py-3 md:px-10 md:py-5 rounded-full font-bold text-center focus:outline-none focus:ring-2 focus:ring-offset-2 tracking-tight";
  const styles = {
  ...
  const MotionWrapper = ({ children: innerChildren }: any) => (
  ...
  if (to) {
    return (
      <MotionWrapper>
        <Link href={to} className={fullClass}>{children}</Link>
      </MotionWrapper>
    );
  }
...
export const FAQAccordion: React.FC<{ question: string; answer: string }> = ({ question, answer }) => {
  ...
  return (
    <div className={`border-b border-slate-200 ${isOpen ? 'py-6' : 'py-2'}`}>
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className="w-full text-left px-4 py-5 flex justify-between items-center group"
      >
        <span className={`font-bold md:text-xl tracking-tight ${isOpen ? 'text-blue-500' : 'text-slate-800'}`}>{question}</span>
        <div 
          className={`${isOpen ? 'text-blue-500 rotate-180' : 'text-slate-400 group-hover:text-slate-600'}`}
        >
          <ChevronDown size={20} />
        </div>
      </button>
...
export const PricingCard: React.FC<{
  ...
  return (
    <div 
      className={`relative p-10 md:p-14 rounded-[3rem] bg-white border ${plan.recommended ? 'border-blue-500 shadow-[0_40px_100px_-15px_rgba(0,0,0,0.1)] z-10' : 'border-slate-100 shadow-sm hover:shadow-xl hover:border-slate-200'} flex flex-col group`}
    >
      {plan.recommended && (
        <div className="absolute -top-4 left-1/2 bg-blue-500 text-white text-[11px] font-bold uppercase tracking-[0.2em] px-8 py-2.5 rounded-full shadow-lg whitespace-nowrap">
          Opción Premium
        </div>
      )}
      ...
      <div className="bg-slate-50 p-6 rounded-[2rem] mb-12 flex items-center gap-5 border border-slate-100 group-hover:bg-blue-50/50">
      ...
      <ul className="space-y-6 mb-14 flex-grow">
        {plan.features.map((f: string, i: number) => (
          <li key={i} className="flex items-start gap-5 text-base md:text-lg text-slate-600 font-medium leading-snug">
            <div className={`mt-1 rounded-full p-0.5 ${plan.recommended ? 'text-blue-500' : 'text-slate-300'}`}>
              <CircleCheck size={20} />
            </div>
            <span>{f}</span>
          </li>
