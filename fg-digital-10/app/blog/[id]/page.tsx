...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `${CANONICAL_DOMAIN}${seoData.path}`,
    },
    openGraph: {
      title: seoData.title,
      description: seoData.description,
      url: `${CANONICAL_DOMAIN}${seoData.path}`,
      type: 'article',
      images: [
        {
          url: post.image,
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
            <aside className="hidden lg:block lg:col-span-1 space-y-6 sticky top-32 h-fit">
              <div className="flex flex-col gap-4">
                <button aria-label="Compartir artículo" className="w-12 h-12 rounded-full border border-slate-200 flex items-center justify-center text-slate-400 hover:text-blue-600 hover:border-blue-600 shadow-sm">
            ...
            <div className="lg:col-span-8 px-2 md:px-0">
              <div className="rounded-[2.5rem] overflow-hidden mb-12 shadow-2xl border-8 border-slate-50 relative aspect-video">
                <Image 
            ...
            <aside className="lg:col-span-3 space-y-12">
              <div className="bg-slate-900 rounded-3xl p-8 text-white shadow-2xl">
                ...
                <div className="space-y-4">
                  <Link href="/servicios" className="flex items-center justify-between p-4 bg-white/5 rounded-xl hover:bg-white/10 border border-white/10 group">
                    <span className="font-bold text-sm">Ver Servicios</span>
                    <ArrowRight size={16} aria-hidden="true" />
                  </Link>
                  <Link href="/tarifas-diseno-web" className="flex items-center justify-between p-4 bg-white/5 rounded-xl hover:bg-white/10 border border-white/10 group">
                ...
                <div className="space-y-6">
                  {BLOG_POSTS.filter(p => p.id !== post.id).slice(0, 2).map(p => (
                    <Link key={p.id} href={`/blog/${p.id}`} className="group block">
                      ...
                      <h5 className="font-bold text-slate-900 leading-tight line-clamp-2">
                        {p.title}
                      </h5>
                    </Link>
                  ))}
