...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: `${CANONICAL_DOMAIN}${METADATA.services.path}`,
  },
};
  ...
  <Section id={id} className={`${bgColor} border-b border-slate-50 py-12 md:py-24`}>
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-start">
      <div className="lg:sticky lg:top-24">
        ...
        {moreInfoPath && (
          <Link href={moreInfoPath} className="inline-flex items-center gap-2 text-blue-600 font-bold hover:underline group">
            {moreInfoLabel} <ArrowRight size={18} className="group-hover:x-1" />
          </Link>
        )}
      </div>
          ...
          <ul className="space-y-6">
            {includes.map((item, i) => (
              <li key={i} className="flex gap-4 items-start group">
                <div className="bg-green-100 p-1.5 rounded-full shrink-0 mt-0.5 border border-green-200 text-green-600 group-hover:bg-green-600 group-hover:text-white">
...
const ServicesPage = () => {
  const serviceListSchema = {
    ...
    "@graph": [
      ...
      {
        "@type": "Service",
        "name": "Diseño Web que Vende",
        "description": "Creamos máquinas de captación que cargan al instante.",
        "provider": { "@id": `${CANONICAL_DOMAIN}/#organization` }
      },
      {
        "@type": "Service",
        "name": "SEO Local",
        "description": "Domina el mapa y las búsquedas en tu ciudad.",
        "provider": { "@id": `${CANONICAL_DOMAIN}/#organization` }
      },
      {
        "@type": "FAQPage",
    ...
    <main className="bg-white overflow-x-hidden">
      ...
      <Section className="pt-6 md:pt-20 pb-8 bg-gradient-to-b from-slate-50 to-white">
        ...
        <div className="max-w-7xl">
          ...
          <p className="text-lg md:text-2xl text-slate-600 max-w-7xl mb-2 leading-relaxed">
      ...
      <Section className="py-20 text-center bg-white">
        <div className="max-w-7xl mx-auto bg-slate-900 text-white p-12 md:p-20 rounded-[3rem] shadow-2xl relative overflow-hidden">
