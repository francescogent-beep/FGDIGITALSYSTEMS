...
import { Info, ArrowRight, CheckCircle2, Zap } from 'lucide-react';
...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: `${CANONICAL_DOMAIN}${METADATA.pricing.path}`,
  },
};
  ...
  return (
    <main>
      ...
      <Section className="pt-6 md:pt-20 bg-white pb-6">
        <Breadcrumbs items={[{ label: 'Nuestros Packs' }]} />
        <div className="text-center">
          <h1 className="text-3xl md:text-5xl font-black mb-4 text-slate-900 tracking-tight">Packs y Soluciones</h1>
          <p className="text-base md:text-xl text-slate-600 max-w-7xl mx-auto leading-relaxed">
            Estructuras de trabajo diseñadas para que recuperes tu inversión atrayendo nuevos clientes. Sin sorpresas ni cuotas ocultas.
          </p>
        </div>
      </Section>

      {/* 2. MAIN PACKS */}
      <Section className="bg-slate-50 border-y border-slate-200/50 py-16 md:py-24">
        <div className="text-center mb-16">
          <p className="text-sm md:text-lg text-slate-500 font-medium italic max-w-7xl mx-auto leading-relaxed">
            “Si dudas entre dos packs, el Pack 2 — Consigue Clientes suele ser el punto de equilibrio perfecto entre coste y resultados.”
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 px-2 md:px-0 mb-20">
        ...
        <div className="max-w-7xl mx-auto mb-24">
          <div className="bg-slate-800 rounded-[3rem] p-10 md:p-20 text-white border border-white/5 shadow-2xl relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-1 bg-blue-500"></div>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-10 mb-12">
              <div>
                <span className="bg-blue-500 text-[11px] font-bold uppercase px-4 py-1.5 rounded-full mb-4 inline-block tracking-widest">Solución Avanzada</span>
                <h3 className="text-4xl md:text-5xl font-bold tracking-tight">{PRICING_PLANS[3].name}</h3>
                <p className="text-slate-400 mt-4 max-w-7xl text-lg font-medium">{PRICING_PLANS[3].description}</p>
              </div>
              <div className="text-left md:text-right">
                <span className="text-slate-500 text-sm block font-bold uppercase tracking-widest mb-1">Inversión</span>
                <span className="text-5xl md:text-6xl font-bold text-blue-500 tracking-tight">Consultar</span>
              </div>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-6 mb-12">
              {PRICING_PLANS[3].features.map((f, i) => (
                <div key={i} className="flex items-center gap-4 text-base text-slate-300 font-medium">
                  <CheckCircle2 size={20} className="text-blue-500 shrink-0" />
                  {f}
                </div>
              ))}
            </div>
            <Button tallyId="q4GKJO" variant="primary" className="w-full md:w-auto px-16 py-6 text-xl shadow-lg">Me interesa el sistema completo</Button>
          </div>
        </div>

        <div className="flex justify-center">
           <div className="bg-blue-50 border border-blue-100 p-5 rounded-xl flex gap-3 max-w-7xl shadow-sm">
              <Info className="text-blue-600 shrink-0" size={20} />
              <p className="text-xs md:text-sm text-blue-800 leading-relaxed font-medium">
                <strong>¿Por qué pago único?</strong> Porque creemos que tu web debe ser un activo de tu negocio, no una suscripción eterna. <Link href="/preguntas-frecuentes" className="font-bold underline">Más info en FAQ</Link>.
              </p>
           </div>
        </div>
      </Section>
      ...
      <Section className="py-12 md:py-24 bg-white">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          <div>
            ...
            <p className="text-slate-600 mb-8 leading-relaxed max-w-7xl">
          ...
          <div className="grid grid-cols-1 gap-3">
             {ADD_ONS.map((addon, i) => (
               <div key={i} className="flex items-center justify-between p-5 bg-slate-50 border border-slate-100 rounded-2xl hover:border-blue-200 group">
                  <div className="flex items-center gap-3">
                     <div className="bg-white p-2 rounded-lg text-blue-600 border border-slate-100 group-hover:bg-blue-600 group-hover:text-white">
                        <CheckCircle2 size={18} />
                     </div>
                     <span className="font-bold text-slate-800 text-sm md:text-base">{addon.name}</span>
                  </div>
                  <span className="font-black text-blue-600">Incluido</span>
               </div>
             ))}
          </div>
        </div>
      ...
      <Section className="bg-slate-50 border-t border-slate-200 py-12 md:py-24">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-600 max-w-7xl mx-auto">Delegación total. Nos encargamos de que tu visibilidad siga creciendo mes a mes.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto">
           {RETAINERS.map((plan, i) => (
             <div 
               key={i} 
               className={`p-8 md:p-10 rounded-[2.5rem] bg-white border ${plan.recommended ? 'border-blue-600 shadow-xl z-10' : 'border-slate-200 shadow-sm'} relative flex flex-col`}
             >
                {plan.recommended && (
                ...
                <div className="mb-8">
                   <span className="text-3xl font-black text-blue-600">Consultar</span>
                </div>
      ...
      <Section className="py-20 text-center bg-slate-50 border-t border-slate-200">
        <div className="bg-slate-900 text-white p-8 md:p-20 rounded-[3rem] shadow-2xl relative overflow-hidden">
          ...
          <p className="text-slate-400 text-lg mb-10 max-w-7xl mx-auto relative z-10 leading-relaxed">
