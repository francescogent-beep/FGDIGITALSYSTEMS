...
import { CheckCircle2, Smartphone, Zap, Search, Lock } from 'lucide-react';
import Link from 'next/link';
import type { Metadata } from 'next';

export const metadata: Metadata = {
  title: METADATA.webQueVende.title,
  description: METADATA.webQueVende.description,
  alternates: {
    canonical: `${CANONICAL_DOMAIN}${METADATA.webQueVende.path}`,
  },
};

const WebQueVendePage = () => {
  const schema = {
    ...
    "@graph": [
      {
        ...
        "itemListElement": [
          ...
          { "@type": "ListItem", "position": 3, "name": "Web que Vende" }
        ]
      },
      {
        "@type": "Service",
        "@id": `${CANONICAL_DOMAIN}${METADATA.webQueVende.path}/#service`,
        "name": "Diseño Web Profesional y Optimizado",
        "description": METADATA.webQueVende.description,
  ...
  return (
    <main className="bg-white">
      ...
      <Section className="pt-20 bg-slate-50">
        <Breadcrumbs items={[{ label: 'Servicios', path: '/servicios' }, { label: 'Web que Vende' }]} />
      ...
      <Section className="bg-slate-50 py-12 md:py-24 border-y border-slate-200">
        ...
        <p className="text-lg text-slate-600 max-w-7xl leading-relaxed mb-10">
          Obtendrás una herramienta de negocio estable and profesional. No prometemos "ser virales", prometemos un sistema que transmita la seriedad de tu empresa y facilite que las personas interesadas en tus servicios darán el primer paso para contactarte.
        </p>
        <Link href="/seo-local" className="text-blue-600 font-bold hover:underline">Ver cómo el SEO Local potencia este sistema</Link>
      </Section>
      ...
      <Section className="pb-24">
        <div className="bg-slate-900 rounded-[3rem] p-12 text-center text-white">
          ...
          <div className="mt-8">
            <Link href="/servicios" className="text-slate-400 hover:text-white">Volver a todos los servicios</Link>
          </div>
        </div>
      </Section>
    </main>
...
export default WebQueVendePage;
