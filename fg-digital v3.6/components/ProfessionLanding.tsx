...
interface ProfessionLandingProps {
  city: string;
  professionSlug: string;
}

const ProfessionLanding: React.FC<ProfessionLandingProps> = ({ city, professionSlug }) => {
  const scrollRef = useRef<HTMLDivElement>(null);
  
  const cityName = formatCityName(city);
  const cityWiki = WIKIDATA_MAP[city] || `https://es.wikipedia.org/wiki/${cityName.replace(' ', '_')}`;
  const config = PROFESSIONS[professionSlug];
  ...
  const dynamicPath = `/${city}/web-seo/${professionSlug}`;
  ...
  const professionSchema = {
    ...
    "@graph": [
      {
        "@type": "BreadcrumbList",
        "@id": `${CANONICAL_DOMAIN}${dynamicPath}/#breadcrumb`,
        "itemListElement": [
      ...
      {
        "@type": "Service",
        "@id": `${CANONICAL_DOMAIN}${dynamicPath}/#service`,
      ...
      {
        "@type": "FAQPage",
        "@id": `${CANONICAL_DOMAIN}${dynamicPath}/#faq`,
        "mainEntity": [
          {
            "@type": "Question",
  ...
  return (
    <main className="bg-white min-h-screen">
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(professionSchema) }}
      />
      ...
      <section className="pt-10 md:pt-20 pb-16 md:pb-24 px-4 text-center bg-white overflow-hidden">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-lg md:text-xl text-slate-600 mb-10 md:mb-12 max-w-7xl mx-auto leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            ...
            <a href="https://wa.me/34694285438" target="_blank" rel="noopener" className="inline-flex items-center justify-center px-10 py-5 bg-green-500 text-white rounded-2xl font-bold hover:bg-green-600 shadow-xl shadow-green-200 gap-2">
              <MessageCircle size={20} /> WhatsApp Directo
            </a>
          </div>

          <div className="mt-12 md:mt-16 text-slate-500 text-xs font-bold uppercase tracking-widest flex flex-wrap items-center justify-center gap-x-8 gap-y-4">
              ...
              <div className="grid gap-4">
                {config.problems.map((p, i) => (
                  <div key={i} className="flex gap-4 p-4 md:p-5 bg-white border border-slate-100 rounded-2xl shadow-sm group hover:border-red-100">
                    <div className="bg-red-50 p-2.5 rounded-xl h-fit text-red-600">{p.icon}</div>
                    <div>
              ...
              <div className="space-y-6 md:space-y-8">
                {[
                  ...
                  <div key={i} className="flex gap-4 md:gap-6 relative group">
                    <div className="flex-shrink-0 w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-white/10 flex items-center justify-center font-black text-white text-lg md:text-xl border border-white/30 group-hover:bg-white group-hover:text-blue-600">
                      {s.step}
                    </div>
                    <div>
      ...
      <Section id="packs" className="bg-slate-50 py-12 md:py-24 border-y border-slate-100">
        <div className="text-center mb-10 md:mb-16">
          ...
          <p className="text-slate-700 text-sm md:text-base max-w-7xl mx-auto">Inversión clara para resultados directos en tu zona.</p>
        </div>
          ...
          <div className="order-2 lg:order-1">
            <div className="relative aspect-video">
              <Image 
                ...
                className="rounded-[2.5rem] shadow-2xl border-8 border-slate-50 object-cover"
                fill
      ...
      <Section className="bg-slate-50 py-12 md:py-24 overflow-hidden border-y border-slate-100">
        <div className="text-center mb-16 px-4">
          <span className="text-blue-600 font-black uppercase tracking-widest text-xs mb-2 block">Reseñas en Google</span>
          <h2 className="text-3xl md:text-5xl font-black text-slate-900 mb-4">Reseñas en Google</h2>
          <p className="text-slate-600 max-w-7xl mx-auto text-sm md:text-base leading-relaxed">Muchos negocios ya han confiado en nuestro sistema para digitalizar su captación de clientes.</p>
        </div>
        
        <div className="relative group max-w-7xl mx-auto px-4">
          <div className="absolute top-1/2 -left-4 md:-left-12 z-10 hidden md:block">
            <button onClick={() => scroll('left')} className="p-3 bg-white border border-slate-200 rounded-full shadow-lg text-slate-400 hover:text-blue-600"><ChevronLeft size={24} /></button>
          </div>
          <div className="absolute top-1/2 -right-4 md:-right-12 z-10 hidden md:block">
            <button onClick={() => scroll('right')} className="p-3 bg-white border border-slate-200 rounded-full shadow-lg text-slate-400 hover:text-blue-600"><ChevronRight size={24} /></button>
          </div>
          ...
          <div ref={scrollRef} className="flex gap-6 overflow-x-auto snap-x snap-mandatory scrollbar-hide pb-8">
            {REVIEWS.map((review, i) => (
              <a 
                ...
                className="flex-shrink-0 w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] snap-start p-8 rounded-3xl bg-white border border-slate-100 flex flex-col hover:shadow-xl group"
              >
                ...
                <div className="flex items-center gap-4 mt-auto">
                  <div className="w-12 h-12 bg-slate-50 border border-slate-200 rounded-full flex items-center justify-center text-blue-600 font-black text-lg group-hover:bg-blue-600 group-hover:text-white">{review.name.charAt(0)}</div>
                  <div>
