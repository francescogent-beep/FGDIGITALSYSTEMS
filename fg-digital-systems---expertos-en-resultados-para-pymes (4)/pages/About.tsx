...
import { ShieldCheck, Zap, TrendingUp, Search, MessageCircle, Layout, MousePointer2, CheckCircle2, PhoneCall } from 'lucide-react';
...
const About: React.FC = () => {
  return (
    <main className="bg-white">
      ...
      <Section className="bg-slate-50 border-y border-slate-100 py-12 md:py-24">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-20 items-center">
          ...
          <div className="lg:col-span-7 order-1 lg:order-2 space-y-6 md:space-y-8">
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 leading-tight">Mi historia <br/><span className="text-blue-600 text-xl md:text-3xl">(y por qué hago esto)</span></h2>
            <div className="space-y-5 text-slate-600 text-base md:text-xl leading-relaxed">
              ...
              <p className="font-black text-slate-900 text-xl md:text-2xl">
      ...
      <Section className="py-12 md:py-24">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-16">
          <div className="space-y-6 md:space-y-8">
            ...
            <p className="text-lg md:text-xl font-black text-slate-900 pt-2">
      ...
      {/* 4. PROCESS SECTION - READABILITY & MOBILE OPTIMIZATION */}
      <Section className="bg-slate-50 border-y border-slate-100 py-12 md:py-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 md:gap-8">
          {[
            ...
            <div key={i} className="space-y-5 p-6 md:p-8 bg-white border border-slate-200 rounded-2xl md:rounded-3xl shadow-sm hover:shadow-md transition-shadow">
              ...
              <h4 className="text-lg md:text-xl font-black text-slate-900 leading-tight">{step.t}</h4>
      ...
      <Section className="py-12 md:py-24 bg-white">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-16 items-center">
          <div>
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 mb-8 uppercase tracking-tight">Lo que nos diferencia</h2>
            <div className="space-y-4 md:space-y-6">
              {[
                { t: "Velocidad sin perder calidad", icon: <Zap size={20}/> },
                { t: "SEO técnico bien hecho desde el inicio", icon: <Search size={20}/> },
                { t: "Estructura pensada para convertir", icon: <TrendingUp size={20}/> },
                { t: "Sistemas simples: WhatsApp, reservas, pagos", icon: <MessageCircle size={20}/> },
                { t: "Enfoque práctico: lo que genera resultados", icon: <ShieldCheck size={20}/> }
              ].map((v, i) => (
                <div key={i} className="flex gap-4 items-center p-4 md:p-5 bg-slate-50 rounded-xl md:rounded-2xl border border-slate-100 group hover:bg-blue-600 transition-all cursor-default">
                  <div className="text-blue-600 bg-white p-2 md:p-3 rounded-lg md:rounded-xl shadow-sm group-hover:text-blue-600 transition-colors">
                    {v.icon}
                  </div>
                  <h4 className="font-bold text-slate-900 text-base md:text-lg group-hover:text-white transition-colors leading-tight">{v.t}</h4>
                </div>
              ))}
            </div>
          </div>
          <div className="bg-blue-50 border border-blue-100 p-8 md:p-16 rounded-[2.5rem] md:rounded-[3rem] text-center">
            <h3 className="text-xl md:text-3xl font-black text-slate-900 mb-4 md:mb-6">Nuestro enfoque: simple, directo, medible</h3>
            <p className="text-slate-600 text-base md:text-lg leading-relaxed mb-8 md:mb-10">
              No prometemos “ser #1 en Google en 7 días”. Eso no es real. Lo que sí hacemos es dejar tu web lista para posicionar correctamente y construir una estructura que convierte.
            </p>
            <div className="flex flex-col gap-3 md:gap-4 text-left">
              <div className="bg-white p-4 md:p-6 rounded-xl md:rounded-2xl border border-blue-200 flex items-center justify-between shadow-sm">
                <span className="font-bold text-slate-900 text-sm md:text-base">Mejorar presencia en Google y Maps</span>
                <CheckCircle2 className="text-green-500 shrink-0" size={20} />
              </div>
              <div className="bg-white p-4 md:p-6 rounded-xl md:rounded-2xl border border-blue-200 flex items-center justify-between shadow-sm">
                <span className="font-bold text-slate-900 text-sm md:text-base">Darte un sistema que puedas usar cada semana</span>
                <CheckCircle2 className="text-green-500 shrink-0" size={20} />
              </div>
            </div>
          </div>
      ...
      {/* 6. FINAL CTA */}
      <Section className="pb-16 md:pb-24 pt-6 md:pt-12 text-center bg-white">
        <div className="max-w-4xl mx-auto bg-slate-900 text-white p-8 md:p-20 rounded-[2.5rem] md:rounded-[4rem] shadow-2xl relative overflow-hidden">
          ...
          <h2 className="text-2xl md:text-6xl font-black mb-6 md:mb-8 leading-tight">Si quieres una web <br className="hidden md:block"/> que venda, hablemos</h2>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-3 md:gap-4">
            <Button to="/contacto" variant="primary" className="px-10 py-4 md:px-12 md:py-5 shadow-2xl shadow-blue-500/30 text-base md:text-lg w-full sm:w-auto">
