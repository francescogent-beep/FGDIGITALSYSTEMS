...
const ProfessionLanding: React.FC = () => {
        ...
        "mainEntity": [
          {
            ...
            "acceptedAnswer": { "@type": "Answer", "text": `Por supuesto. Tu web propia y ${config.platform} pueden convivir perfectamente. La idea es que con el tiempo, la web sea tu fuente principal para que no dependas de portales de terceros.` }
          },
          {
            "@type": "Question",
            "name": "¿Cuánto tarda en aparecer en Google Maps?",
            "acceptedAnswer": { "@type": "Answer", "text": `Normalmente, los primeros resultados en Google Maps para tu zona de ${cityName} se notan en 4-8 semanas tras la optimización técnica inicial.` }
          }
        ]
      }
    ]
  ...
  return (
    <main className="bg-white min-h-screen">
      ...
      {/* 1. Hero */}
      <section className="hero-section pt-12 md:pt-32 pb-16 bg-white text-center">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="mt-10 md:mt-12 text-slate-500 text-xs font-bold uppercase tracking-widest flex flex-wrap items-center justify-center gap-x-8 gap-y-3">
            ...
            <span className="flex items-center gap-2"><CircleCheck size={16} className="text-green-600" aria-hidden="true" /> Sin comisiones</span>
          </div>
        </div>
      </section>
        ...
        <div className="relative group max-w-6xl mx-auto px-4">
          ...
          <div ref={scrollRef} className="flex gap-6 overflow-x-auto snap-x snap-mandatory scrollbar-hide pb-8">
            {REVIEWS.map((review, i) => (
              <div key={i} className="flex-shrink-0 w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] snap-start p-8 rounded-3xl bg-white border border-slate-100 flex flex-col hover:shadow-xl transition-shadow duration-300 group">
