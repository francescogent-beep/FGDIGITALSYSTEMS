    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-4 bg-slate-900/90 backdrop-blur-xl transition-all duration-300 animate-in fade-in">
      <div className="bg-white w-full h-full md:h-[85vh] md:max-w-2xl md:max-h-[90vh] md:rounded-[2.5rem] shadow-[0_32px_128px_-16px_rgba(37,99,235,0.25)] flex flex-col overflow-hidden relative border-none md:border md:border-slate-100">
        ...
        <div className="px-6 py-4 md:px-10 md:py-6 border-b border-slate-50 flex items-center justify-between bg-white shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[9px] md:text-[10px] font-extrabold text-slate-600 uppercase tracking-widest mt-0.5">Propuesta Personalizada</span>
            </div>
          </div>
          <button 
            onClick={() => setIsOpen(false)}
            className="p-2 md:p-3 rounded-full bg-slate-50 text-slate-600 hover:text-slate-900 hover:bg-slate-100 transition-all active:scale-90"
            aria-label="Cerrar"
          >
            <X size={24} />
          </button>
        ...
        <div className="hidden md:block px-10 py-4 bg-slate-50 text-center border-t border-slate-100">
          <p className="text-[10px] text-slate-700 font-black uppercase tracking-[0.2em]">Respuesta garantizada en menos de 24 horas</p>
        </div>
      </div>
    </div>
  );
...
export const Button: React.FC<{
  ...
  const styles = {
    ...
    outline: "border-2 border-slate-300 text-slate-900 hover:border-blue-600 hover:text-blue-600 hover:bg-blue-50/40"
  };
...
export const FAQAccordion: React.FC<{ question: string; answer: string }> = ({ question, answer }) => {
  ...
  return (
    <div className={`border rounded-2xl overflow-hidden bg-white mb-4 transition-all duration-300 ${isOpen ? 'border-blue-200 shadow-lg' : 'border-slate-200 hover:border-slate-300'}`}>
      <button 
      ...
      >
        ...
        <div className={`p-2 rounded-full transition-all duration-300 ${isOpen ? 'bg-blue-600 text-white rotate-180' : 'bg-slate-100 text-slate-600 group-hover:text-slate-900'}`}>
          <ChevronDown size={20} />
        </div>
      </button>
...
export const Breadcrumbs: React.FC<{
  ...
  <nav className="flex mb-8 overflow-x-auto whitespace-nowrap pb-2 scrollbar-hide">
    <ol className="flex items-center space-x-2 text-xs md:text-sm font-black tracking-widest uppercase text-slate-600">
      <li>
  ...
  return (
    <div className={`relative p-8 md:p-12 rounded-[2.5rem] bg-white border transition-all duration-500 ${plan.recommended ? 'border-blue-600 shadow-2xl lg:scale-[1.05] z-10' : 'border-slate-200 shadow-lg hover:shadow-xl hover:border-slate-300'} flex flex-col group`}>
      ...
      <div className="mb-8">
        ...
        {plan.tagline && (
          <p className="text-slate-800 text-[11px] font-black uppercase tracking-widest mb-4 italic opacity-80">"{plan.tagline}"</p>
        )}
        <div className="inline-flex items-center gap-2 text-blue-800 text-[10px] font-black uppercase tracking-wider px-4 py-1.5 bg-blue-50 rounded-full border border-blue-100">
      ...
      <div className="mb-8 flex items-baseline gap-2">
        <span className="text-slate-600 text-sm font-bold italic">Desde</span>
        <span className="text-5xl md:text-6xl font-black text-slate-900 tracking-tighter">{plan.price}</span>
      </div>
      
      <p className="text-slate-800 text-sm md:text-base mb-10 min-h-[56px] leading-relaxed font-medium">{plan.description}</p>
      ...
      <div className="bg-slate-50 p-5 rounded-2xl mb-10 flex items-center gap-4 border border-slate-100 group-hover:bg-blue-50/50 transition-colors">
         ...
         <div>
            ...
            <span className="text-[11px] text-slate-800 font-bold opacity-70">Pago único, sin sorpresas.</span>
         </div>
      </div>

      <ul className="space-y-5 mb-12 flex-grow">
        {plan.features.map((f: string, i: number) => (
          <li key={i} className="flex items-start gap-4 text-sm md:text-base text-slate-900 font-bold leading-snug">
