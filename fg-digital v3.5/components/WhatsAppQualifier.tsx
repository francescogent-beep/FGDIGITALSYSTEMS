...
const QUESTIONS = [
  ...
  },
  {
    ...
    options: ['Básico', 'Intermedio', 'Avanzado', 'Corporativo', 'No lo sé aún']
  },
  {
    id: 'q4',
    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-40">
        <button
          ...
          className="relative bg-slate-900 text-white p-4 md:px-6 md:py-4 rounded-full shadow-2xl flex items-center gap-2 group border border-white/10"
          aria-label="Abrir calificador de presupuesto por WhatsApp"
        >
          <MessageCircle size={24} />
          <span className="hidden md:inline font-semibold uppercase tracking-tight text-sm">¿Dudas? Hablamos</span>
        </button>
      </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-md">
          <div className="bg-white rounded-[2rem] w-full max-w-lg shadow-[0_40px_100px_-20px_rgba(0,0,0,0.2)] flex flex-col overflow-hidden border border-slate-200" role="dialog" aria-labelledby="qualifier-title" aria-modal="true">
            ...
            <div className="p-6 md:p-8 flex justify-between items-center border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 id="qualifier-title" className="text-xl font-bold text-slate-800 tracking-tight">Presupuesto Rápido</h3>
                  <p className="text-slate-500 text-[10px] font-semibold uppercase tracking-widest">Paso {Math.min(step + 1, QUESTIONS.length)} de {QUESTIONS.length}</p>
                </div>
              </div>
              <button 
            ...
            {/* Content Body */}
            <div className={`flex-grow p-6 md:p-8 min-h-[360px] flex flex-col ${isAnimating ? 'opacity-0' : 'opacity-100'}`}>
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {QUESTIONS[step].options.map((opt) => (
                      <button
                        ...
                        className={`group flex items-center justify-between p-4 rounded-2xl border text-left ${
                      ...
                      >
                        ...
                        <div className={`w-6 h-6 rounded-full border flex items-center justify-center ${
                          answers[QUESTIONS[step].id] === opt ? 'bg-white border-white text-blue-500' : 'border-slate-300 bg-white'
                        }`}>
                ...
                <div className="flex-grow space-y-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none text-slate-800 font-bold"
                    />
                  </div>
                </div>
              )}
              ...
              <div className="mt-auto pt-6 flex flex-col gap-4">
                <div className="flex gap-3 items-center">
                  {step > 0 && (
                    <button 
                      onClick={prevStep}
                      className="p-4 rounded-2xl text-slate-500 hover:text-slate-900 hover:bg-slate-100 flex items-center justify-center border border-transparent hover:border-slate-200"
                  ...
                  {step < QUESTIONS.length ? (
                    <button
                      ...
                      onClick={nextStep}
                      className={`flex-grow py-4 rounded-2xl font-black flex items-center justify-center gap-2 ${
                    ...
                    <button
                      onClick={handleSend}
                      className="flex-grow py-4 rounded-2xl bg-green-700 text-white font-black hover:bg-green-800 shadow-xl shadow-green-200 flex items-center justify-center gap-2 text-lg"
                    >
