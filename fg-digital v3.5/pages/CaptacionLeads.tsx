...
const CaptacionLeads: React.FC = () => {
  const schema = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 3, "name": "Captación de Clientes", "item": `${CANONICAL_DOMAIN}/captacion-de-clientes` }
    ]
  };

  return (
    <main className="bg-white">
      <SEO {...METADATA.captacionLeads} schema={schema} />
      
      <Section className="pt-20 bg-slate-50">
        <Breadcrumbs items={[{ label: 'Servicios', path: '/servicios-diseno-web-seo' }, { label: 'Captación de Clientes' }]} />
      ...
      <Section className="bg-slate-50 py-12 md:py-24 border-y border-slate-200">
        ...
        <Link to="/mantenimiento-web-seo" className="text-blue-600 font-bold hover:underline">Soporte continuo para mejorar la captación</Link>
      </Section>
      ...
      <Section className="pb-24">
        <div className="bg-slate-900 rounded-[3rem] p-12 text-center text-white">
          ...
          <div className="mt-8">
            <Link to="/servicios-diseno-web-seo" className="text-slate-400 hover:text-white">Volver a servicios</Link>
          </div>
        </div>
      </Section>
    </main>
